...
export function FrenchLegalDocumentWizard() {
  const [step, setStep] = useState<
    "selection" | "questions" | "preview" | "completed"
  >("selection");
  const [selectedDocument, setSelectedDocument] = useState<string>("");
  const [currentTemplate, setCurrentTemplate] =
    useState<LegalDocumentTemplate | null>(null);
  const [currentQuestion, setCurrentQuestion] =
    useState<LegalDocumentQuestion | null>(null);
  ...
  const availableDocuments =
    frenchLegalDocumentGenerator.getAvailableDocuments();

  // Charger le template sélectionné
  useEffect(() => {
    if (selectedDocument) {
      const template =
        frenchLegalDocumentGenerator.getDocumentTemplate(selectedDocument);
      setCurrentTemplate(template);
      if (template) {
        const firstQuestion = frenchLegalDocumentGenerator.getNextQuestion(
          selectedDocument,
          {}
        );
  ...
  useEffect(() => {
    if (selectedDocument && step === "questions") {
      const nextQuestion = frenchLegalDocumentGenerator.getNextQuestion(
        selectedDocument,
        answers
      );
  ...
  }, [answers, selectedDocument, step]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleAnswerSubmit = () => {
    ...
    // Validation
    const validation = frenchLegalDocumentGenerator.validateAnswer(
      currentQuestion,
      currentAnswer
    );
  ...
  const generateDocument = async () => {
    ...
    setIsLoading(true);
    try {
      const result = frenchLegalDocumentGenerator.generateDocument(
        selectedDocument,
        answers
      );
  ...
  const downloadDocument = () => {
    ...
    link.href = url;
    link.download = `${currentTemplate?.name.replace(/[^a-zA-Z0-9]/g, "_")}_${
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-xl text-gray-700">
            Assistant conversationnel pour la création de documents juridiques
            français
          </p>
        </div>
        ...
        {step === "selection" && (
          <div className="bg-white/90 backdrop-blur-sm rounded-xl p-8 shadow-lg border border-gray-200">
            ...
            <p className="text-gray-600 mb-8">
              ...
              Je vous poserai ensuite quelques questions pour le personnaliser
              selon vos besoins.
            </p>
            ...
            <div className="bg-white/90 backdrop-blur-sm rounded-xl p-6 shadow-lg border border-gray-200">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="text-sm text-gray-600">
                  Progression:{" "}
                  {frenchLegalDocumentGenerator.getCompletionRate(
                    selectedDocument,
                    answers
                  )}
                  %
              ...
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div
                  ...
                  style={{
                    width: `${frenchLegalDocumentGenerator.getCompletionRate(
                      selectedDocument,
                      answers
                    )}%`,
                  }}
