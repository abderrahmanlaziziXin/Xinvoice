...
export function AILegalDocumentChat() {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      role: 'assistant',
      content: 'Bonjour ! Je vais vous aider à générer un document juridique. Quel type de document souhaitez-vous créer ? (contrat de travail, bail d\'habitation, contrat de vente, procuration, etc.)',
      timestamp: new Date(),
      suggestions: [
  ...
  const [conversationPhase, setConversationPhase] = useState<'selection' | 'questions' | 'generation' | 'finalization'>('selection');
  ...
  const downloadDocument = async (format: 'docx' | 'pdf' | 'txt') => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        link.href = url;
        link.download = `document_juridique_${new Date().toISOString().split('T')[0]}.${format}`;
        link.click();
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-xl text-gray-700">
            Conversation intelligente avec l'IA pour créer vos documents juridiques français
          </p>
        </div>
              ...
              <div className="p-4 border-t border-gray-200">
                <div className="flex space-x-3">
                  <input
                    ...
                    onKeyPress={(e) => e.key === 'Enter' && !isTyping && currentMessage.trim() && sendMessage(currentMessage)}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white/90 backdrop-blur-sm rounded-xl p-6 shadow-lg border border-gray-200">
              ...
              <div className="space-y-3">
                <div className={`flex items-center space-x-2 ${conversationPhase === 'selection' ? 'text-blue-600 font-medium' : 'text-gray-600'}`}>
                  <div className={`w-3 h-3 rounded-full ${conversationPhase === 'selection' ? 'bg-blue-600' : 'bg-gray-300'}`}></div>
                  <span>Sélection du document</span>
                </div>
                <div className={`flex items-center space-x-2 ${conversationPhase === 'questions' ? 'text-blue-600 font-medium' : 'text-gray-600'}`}>
                  <div className={`w-3 h-3 rounded-full ${conversationPhase === 'questions' ? 'bg-blue-600' : 'bg-gray-300'}`}></div>
                  <span>Collecte d'informations</span>
                </div>
                <div className={`flex items-center space-x-2 ${conversationPhase === 'generation' ? 'text-blue-600 font-medium' : 'text-gray-600'}`}>
                  <div className={`w-3 h-3 rounded-full ${conversationPhase === 'generation' ? 'bg-blue-600' : 'bg-gray-300'}`}></div>
                  <span>Génération IA</span>
                </div>
                <div className={`flex items-center space-x-2 ${conversationPhase === 'finalization' ? 'text-blue-600 font-medium' : 'text-gray-600'}`}>
                  <div className={`w-3 h-3 rounded-full ${conversationPhase === 'finalization' ? 'bg-blue-600' : 'bg-gray-300'}`}></div>
            ...
            {documentData && (
              <div className="bg-white/90 backdrop-blur-sm rounded-xl p-6 shadow-lg border border-gray-200">
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    <span className="text-sm font-medium text-gray-600">Champs complétés :</span>
                    <p className="text-gray-900">{Object.keys(documentData.fields).length}</p>
                  </div>
                  <div>
                    <span className="text-sm font-medium text-gray-600">Suggestions IA :</span>
                    <p className="text-gray-900">{Object.keys(documentData.aiSuggestions).length}</p>
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="bg-white/90 backdrop-blur-sm rounded-xl p-6 shadow-lg border border-gray-200">
                ...
                <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                  <p className="text-sm text-yellow-800">
                    <span className="font-medium">💡 Conseil :</span> Le format DOCX est recommandé pour éditer le document. Le PDF est idéal pour l'impression et la signature.
                  </p>
                </div>
              </div>
            )}
